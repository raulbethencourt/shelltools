...
```vim
" The \U chane the pattern (&) to uppercase
:%s/[Hh]elp/\U&/g 
```

- Replace one or more spaces following a colon or period with two spaces

```vim
:%s/\([:.]\)  */\1  /g 
```

- Delete all blank lines, plus any lines that contian tab or white spaces
...
- `:g/^WARNING:/s/\<not\>/NOT/` - Find lines with "WARNING:" and do replace
